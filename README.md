...
| --- | --- |
| **Main** | The pump itself: operating priority and mode, the Smart Price Adaption master switch, diagnostics, runtime statistics, phase currents, ground-source (brine) temperatures, **alarms with readable descriptions**, compressor status — plus **total energy produced/consumed**, **Total COP**, **idle energy** (standby draw), and the pump's firmware/type in settings. |
...
- **Smart Price Adaption** — the whole-pump on/off (menu 7.1.10). Each function device carries its own
  on/off and influence, following that function's own feature selection; this is the master above them.
...
- **Smart Price Adaption** — on/off for heating, and how strongly the electricity price may move the indoor
  temperature (1–10). The whole-pump master lives on Main; every function device carries this same pair, so
  the feature reads identically wherever you open it.
