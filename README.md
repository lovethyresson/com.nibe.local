...
| Version | Highlights |
|---|---|
| **0.9.13** | The heating device joins Homey's Climate view and renders the thermostat tile. Homey decides both on the *bare* capability ids — a device exposing only `measure_temperature.something` is skipped by Climate entirely, and the tile needs a matching `measure_temperature` + `target_temperature` pair — so room temperature was renamed to the root id (the stored selection is migrated onto the new name, or an S735 would lose the alternate address detection resolved for it and read the sentinel at 111 again). The setpoint is holding **2505**, "Desired room temperature for zone 1": named only in the S735 CSV, present as the untitled `id:12801` on all six S models, and the same setting as pump menu 1.1 and myUplink's 47751 — confirmed by all three reading 22.0 at once. It is *not* register 206, which sat at its untouched 20.0 default on the same pump, alongside "Use room sensor" (202) reading 0 while the pump's menu 1.3.3 visibly had BT50 controlling heating; on this generation the 199–210 climate-system block does not describe a zone-based pump. Writing it needed a first: **32-bit writes**, and the pump turns out to accept only a two-word FC16 request assembled **high word first** — the opposite of the order it reads in. Low-word-first writes and single-word FC06 writes are all accepted without a Modbus exception and silently discarded, so this was measured, not deduced. Registers can now also declare a plausibility band for their own value: unconfigured zones answer 0 rather than the sentinel, so a pump without a room zone is offered no dial instead of one reading 0 °C. Main joins Climate as well, mirroring the outdoor sensor into the bare id — a mirror rather than a second register, since a register's name *is* its capability id and heating already holds that one. Publishing it is also what gives the owner Homey's per-device "Include in Climate" toggle, which is the escape hatch for the trade it carries: a device reporting outdoor temperature pulls the average of whatever zone it sits in, so Main belongs in an outdoor zone. Evidence from [halderex's S735](https://github.com/lovethyresson/com.nibe.local/issues/4). |
